...
async function all(
    ...
    query.attributes = [
        ...
        [
            Sequelize.literal(`(
                        ...
                        WHERE
                            logs.account_category_id = AccountCategoryModel.id
                            AND
                            logs.type = "income"
                    )`),
            'total_income',
    ...
    try {
        ...
        data.total_income = await models.AccountLogModel.sum('amount',{
            where:{
                type: 'income',
            }
        });
        
        data.total_expense = await models.AccountLogModel.sum('amount',{
            where:{
                type: 'expense',
            }
        });
